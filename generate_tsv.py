...
def parse_args():
    ...
    parser.add_argument(
        ...
        dest="data_split",
        help="dataset to use",
        default="karpathy_train",
        type=str,
    )
    parser.add_argument(
        "--id_txt",
        dest='id_txt',
        help='output image ids text file',
        type=str,
    )
...
def get_detections_from_im(
    ...
    if len(keep_boxes.shape) == 0 or keep_boxes.shape[0] == 0 or len(keep_boxes) < MIN_BOXES:
    ...
    # # load class names, and get object labels/
    # classes = ["__background__"]
    # with open(os.path.join(args.classes_dir, "objects_vocab.txt")) as f:
    #     for object in f.readlines():
    #         classes.append(object.split(",")[0].lower().strip())
    # NOTE: 0th class is __background__ pseudo-class.
    objects = torch.argmax(scores[keep_boxes][:, 1:], dim=1)
    object_classes = []
    for object_kind in objects:
        object_classes.append(classes[object_kind + 1])
    # ic(object_classes)
    ...
    return {
        ...
        "boxes": base64.b64encode(box_dets), # seems BB features
        "tags": object_classes,
...
if __name__ == "__main__":
    ...
    print(args)
    images = glob.glob(os.path.expanduser(args.image_dir))
    assert images, "The input path(s) was not found"
    # image_ids = load_image_ids(args.data_split)
    # image_ids = [["images/img1.jpg", 0], ["images/img2.jpg", 1]]
    sid = [get_id(image_path) for image_path in images]
    print('Saving ID file to')
    ic(args.id_txt)
    with open(args.id_txt, 'w') as f:
        f.writelines([str(x) + '\n' for x in sid])
    image_ids = list(zip(images, sid))
    # ic(image_ids)
